...
<svelte:head>
	...
	<meta name="description" content="Writing on software engineering, startups, and indie hacking." />
</svelte:head>
		...
		{#each data.posts as post, id}
			<BlurFade delay={BLUR_FADE_DELAY * 1.5 + id * 0.05}>
				...
				>
					...
					<div class="space-y-3">
						<h2 class="font-serif text-xl font-semibold transition-colors duration-300 group-hover:text-gold sm:text-2xl">
							{post.title}
						</h2>
					...
					<!-- Arrow indicator -->
					<div class="absolute right-6 top-1/2 -translate-y-1/2 opacity-0 transition-all duration-300 group-hover:translate-x-1 group-hover:opacity-100">
						<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="text-gold">
