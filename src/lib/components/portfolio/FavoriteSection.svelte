<script lang="ts">
	...
	let favorites = $derived(DATA.favorites as FavoriteItem[]);
</script>
		...
		{#each favorites as item, i}
			...
			{#if item.isSpotify}
				...
				>
					<Dialog.Root>
						<Dialog.Trigger
							class="favorite-card-inner relative flex h-full cursor-pointer flex-col justify-between overflow-hidden rounded-2xl border-0 bg-transparent p-0 text-left transition-all duration-500 ease-out group-hover:-translate-y-1 group-hover:shadow-2xl group-hover:shadow-black/30"
						>
							<img
