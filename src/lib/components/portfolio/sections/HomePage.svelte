<script lang="ts">
	...
	function loadContributions() {
		fetch(`${base}/api/contributions`)
			.then((r) => r.json())
			.then((d) => { contribData = d; contribLoading = false; })
			.catch(() => { contribError = true; contribLoading = false; });
	}
...
{/if}
	...
	<main id="main">
		<section id="top" data-chapter="序" class="hero">
			...
			<div class="hero-left">
				...
				<ul class="hero-bullets">
					<li><span class="arrow">→</span> 构建 <b>AI 智能体</b><span class="tag">LangGraph · MCP</span></li>
					<li><span class="arrow">→</span> 参与 <b>Monad Blitz</b> 原型<span class="tag">48 小时冲刺</span></li>
					<li><span class="arrow">→</span> <b>{DATA.repoCount} 个仓库</b> 持续增长<span class="tag">一直在打造</span></li>
				</ul>
			</div>
			<nav class="index" aria-label="Contents">
				<button class="seal-trigger" type="button" onclick={() => window.dispatchEvent(new CustomEvent('living-seal:impulse'))}>
			...
			<div class="self-grid">
				...
				<aside>
					...
					{#each DATA.education as e}
						<div class="edu">
							{#if e.logoUrl}<img class="edu-logo" src={e.logoUrl} alt={e.school} width="42" height="42" loading="lazy" />{/if}
							<div><a href={e.href}>{e.school}</a><br /><em>{e.degree}</em><br /><span class="years">{e.start} — {e.end}</span></div>
						</div>
					{/each}
				</aside>
		...
		<section id="exp" data-chapter="歷" class="sec">
				...
				<ol class="timeline">
					{#each work as w}
						<li class="tl-item">
							...
							<div class="tl-body">
								<div class="tl-top">
									{#if w.logoUrl}<img class="tl-logo" src={w.logoUrl} alt={w.company} width="44" height="44" loading="lazy" />{/if}
									<div>
			...
			{:else}
				<div class="exp-open">
					...
					<div class="exp-grid">
						<div class="exp-stat">
							<span class="exp-num"><span class="count" data-count={DATA.repoCount}>{DATA.repoCount}</span></span>
					...
					<div class="exp-actions">
						...
						<a class="exp-cta" href={`${base}/resume.pdf`} target="_blank" rel="noreferrer">下载简历 ↓</a>
					</div>
					<p class="exp-note">
					...
					<li class="row featured" style={`--project-index:${i}`}>
						...
						{#if p.image}
							<a class="row-thumb" href={p.href} target="_blank" rel="noreferrer" tabindex="-1" aria-hidden="true">
								<img src={img(p.image)} alt="" width="320" height="200" loading="lazy" decoding="async" fetchpriority="low" onerror={(e) => onImgError(e, p.title)} />
							</a>
						{/if}
					</li>
				{/each}
					...
					<li class="hx">
						...
						<div class="hx-body">
							...
							<div class="hx-links">
								...
								{#if h.links.length === 0}
									<a href={`mailto:${DATA.contact.email}?subject=${encodeURIComponent(h.title + ' details')}`}>Ask for details →</a>
								{/if}
							</div>
						</div>
					</li>
		...
		<section id="off" data-chapter="閒" class="sec">
			...
			<ul class="track" bind:this={animeTrack} onscroll={updateAnimeProg} onmouseenter={() => animeAuto(false)} onmouseleave={() => autoOK && animeAuto(true)} onkeydown={onAnimeKey} role="region" aria-label="Anime collection" tabindex="0">
					...
					<li class="acard">
						<a href={a.link} target="_blank" rel="noreferrer" aria-label={a.name}>
							<Lens zoomFactor={1.5} lensSize={150} class="rounded-none">
								{#snippet children()}
									<div class="frame">
										<img src={img(a.image)} alt={a.name} width="280" height="373" loading="lazy" decoding="async" fetchpriority="low" />
										<span class="anime-no">{String(DATA.anime.indexOf(a) + 1).padStart(2, '00')}</span>
				...
				{#each DATA.favorites as f, i}
					<li class="fav" class:featured-fav={i === 0} style="--fav-delay: {i * 0.06}s">
						<a href={f.href} target="_blank" rel="noreferrer" class="fav-link">
							<div class="fav-img">
								<img src={img(f.background)} alt={f.name} width="400" height="225" loading="lazy" decoding="async" fetchpriority="low" />
							</div>
							<div class="fav-cap">
...
<style>
	...
	.seal-fallback {
		...
		opacity: 0.13; pointer-events: none; mix-blend-mode: multiply;
	}
	.fallback-loop {
		position: absolute; left: 50%; top: 0; width: 34%; aspect-ratio: 1;
		border: clamp(5px, 0.7vw, 10px) solid var(--zhu); border-radius: 50%;
		transform: translateX(-50%);
		box-shadow: inset 0 0 0 clamp(4px, 0.5vw, 8px) var(--paper), inset 0 0 0 clamp(6px, 0.8vw, 12px) var(--ink);
	}
	.fallback-body {
		position: absolute; inset: 22% 10% 0;
		display: grid; grid-template-rows: 1fr 1.25fr 1.5fr; gap: 5%;
	...
	.skip-link {
		position: absolute; top: -100%; left: var(--page-x); z-index: 9999;
		padding: 8px 16px; background: var(--ink); color: var(--paper);
		font-family: var(--font-label); font-size: 0.8rem; font-weight: 700;
		letter-spacing: 0.08em; text-decoration: none; border-radius: 0 0 4px 4px;
		transition: top 0.2s ease;
	}
	.skip-link:focus { top: 0; outline: 2px solid var(--zhu); outline-offset: -2px; }
	...
	.ink-wash {
		...
		background:
			radial-gradient(50% 45% at 88% 8%, var(--ink-wash-1, rgba(26, 24, 21, 0.14)), transparent 70%),
			radial-gradient(38% 38% at 12% 96%, var(--ink-wash-2, rgba(198, 65, 44, 0.08)), transparent 70%);
		filter: blur(30px);
	}
	...
	.logo .word { font-weight: 900; font-optical-sizing: auto; font-size: clamp(2.5rem, 6vw, 4rem); line-height: 0.8; letter-spacing: -0.02em; }
	.seal {
		display: grid; place-items: center; width: 58px; height: 58px; border-radius: 8px;
		background: var(--zhu); color: var(--paper); font-family: var(--font-serif);
		font-optical-sizing: auto; font-weight: 900; font-style: normal;
		font-size: 22px; line-height: 1; letter-spacing: -0.04em; text-align: center;
		transform: rotate(-4deg); box-shadow: inset 0 0 0 2.5px rgba(243, 239, 230, 0.32);
		filter: blur(0px); transition: filter 0.6s ease;
	}
	.ed-line {
		text-align: right; font-family: var(--font-label); font-size: var(--type-label);
		letter-spacing: 0.16em; text-transform: uppercase; line-height: 1.7; color: var(--ink-soft);
	}
	.ed-line b { color: var(--ink); }
	.band {
		display: flex; flex-wrap: wrap; gap: 16px 32px; justify-content: space-between;
		padding: 10px 0; font-family: var(--font-label); font-size: var(--type-label);
		letter-spacing: 0.26em; text-transform: uppercase; color: var(--ink-soft);
	}
	.band .z { color: var(--ink); }

	.hero {
		display: grid; grid-template-columns: 1.6fr 1fr; gap: clamp(32px, 5vw, 72px);
		padding: clamp(40px, 8vh, 96px) 0 var(--page-y); align-items: center;
		position: relative; overflow: hidden;
	}
	.hero-left { position: relative; }
	.hero-left::before {
		content: ''; position: absolute; bottom: -6%; left: -8%; width: 52%; height: 58%;
		background: radial-gradient(circle, rgba(198, 65, 44, 0.16), transparent 70%);
		filter: blur(55px); z-index: -1; pointer-events: none;
	}
	.hero-meta { display: flex; align-items: center; gap: 16px; margin-bottom: 26px; }
	.hero-meta .rule { flex: 1; height: 1.5px; background: var(--ink-line); }
	.hero-meta .folio {
		font-family: var(--font-label); font-weight: 700; font-size: var(--type-label);
		letter-spacing: 0.16em; color: var(--ink-mute);
	}
	.tag {
		font-family: var(--font-label); font-weight: 700; font-size: var(--type-label);
		letter-spacing: 0.3em; text-transform: uppercase; color: var(--ink); white-space: nowrap;
	}
	h1 {
		font-family: var(--font-serif); font-weight: 900; font-optical-sizing: auto;
		font-size: clamp(4rem, 11.5vw, 11rem); line-height: 0.82; letter-spacing: -0.03em; margin: 0;
	}
	h1 .em {
		font-weight: 700; font-style: normal; font-size: clamp(2.75rem, 7vw, 6.5rem);
		color: var(--zhu); position: relative; display: inline-block; padding-bottom: 0.12em; letter-spacing: -0.02em;
	}
	h1 .em::after {
		content: ''; position: absolute; left: -2%; right: -2%; bottom: 0.04em; height: 0.14em;
		background: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 200 12' preserveAspectRatio='none'><path d='M2 8 C 40 3 90 9 140 4 S 190 3 198 6' fill='none' stroke='%231a1815' stroke-width='1.4' stroke-linecap='round' opacity='0.7'/></svg>") no-repeat center / 100% 100%;
		pointer-events: none;
	}
	h1 .z { font-weight: 700; font-size: clamp(2.75rem, 7vw, 6.5rem); color: var(--zhu); letter-spacing: -0.02em; }
	.dek { font-size: var(--type-dek); line-height: 1.65; max-width: 46ch; margin-top: 26px; color: var(--ink); }
	.hero-divider { width: 48px; height: 1.5px; background: var(--ink); opacity: 0.4; margin-top: 28px; margin-bottom: 22px; }

	.hero-bullets { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 10px; }
	.hero-bullets li { display: flex; align-items: baseline; gap: 10px; flex-wrap: wrap; font-size: 1.02rem; line-height: 1.45; color: var(--ink); }
	.hero-bullets .arrow { color: var(--zhu); font-weight: 700; flex: 0 0 auto; }
	.hero-bullets b { font-weight: 700; }
	.hero-bullets .tag {
		font-family: var(--font-label); font-size: 0.68rem; letter-spacing: 0.12em;
		text-transform: uppercase; color: var(--ink-mute); padding-left: 8px; border-left: 1px solid var(--ink-line);
	}

	.index {
		border-left: 1.5px solid var(--ink-line); margin-top: 210px; padding-left: clamp(18px, 2vw, 30px);
		padding-right: clamp(10px, 1.4vw, 20px);
		background: linear-gradient(90deg, color-mix(in srgb, var(--paper) 94%, transparent) 0 48%, color-mix(in srgb, var(--paper) 26%, transparent) 100%);
	}
	.index-h {
		font-family: var(--font-label); font-weight: 700; font-size: var(--type-label);
	...
	.seal-trigger {
		width: 100%; display: flex; align-items: center; gap: 9px; margin: 0 0 14px; padding: 10px 0;
		border: 0; border-top: 1px solid var(--ink-line-strong); border-bottom: 1px solid var(--ink-line-strong);
		background: color-mix(in srgb, var(--paper) 68%, transparent); color: var(--ink);
		font-family: var(--font-label); font-size: 0.67rem; font-weight: 700;
		letter-spacing: 0.19em; text-transform: uppercase; cursor: pointer;
		backdrop-filter: blur(7px); transition: color 180ms ease, letter-spacing 280ms ease;
	}
	.seal-trigger:hover { color: var(--zhu); letter-spacing: 0.22em; }
	.seal-trigger > :last-child { margin-left: auto; }
	.seal-trigger-dot {
		width: 7px; height: 7px; border-radius: 50%; background: var(--zhu);
		box-shadow: 0 0 0 0 rgba(192, 57, 43, 0.34); animation: sealPulse 2.4s ease-out infinite;
	}
	@keyframes sealPulse {
		60%, 100% { box-shadow: 0 0 0 9px rgba(192, 57, 43, 0); }
	}
	.index ul { list-style: none; margin: 0; padding: 0; }
	.index li a {
		display: flex; align-items: baseline; justify-content: space-between; gap: 12px;
		padding: 10px 0; border-bottom: 1px solid var(--ink-line); transition: color 0.2s;
	}
	.index li a:hover { color: var(--zhu); }
	.index .n { font-family: var(--font-label); font-weight: 700; font-size: 0.7em; color: var(--ink-mute); }
	.index em { font-style: italic; color: var(--ink-soft); font-size: 0.85em; }
	.index .p {
		font-family: var(--font-label); font-weight: 700; font-size: 0.7em;
		color: var(--ink-soft); font-variant-numeric: tabular-nums;
	}
	.index .p-sep { opacity: 0.4; margin: 0 2px; }
	.index .p-total { opacity: 0.55; }
	.pull { font-style: italic; font-size: 1.15rem; line-height: 1.35; margin-top: 26px; }
	.pull b { font-style: normal; color: var(--ink); border-bottom: 2px solid var(--zhu); padding-bottom: 1px; }
	.now-link {
	...
	.now-link:hover { color: var(--zhu); border-color: var(--zhu); gap: 12px; }
	.now-link .now-arrow { transition: transform 0.25s ease; }
	.now-link:hover .now-arrow { transform: translateX(2px); }
	.hero-watermark {
		position: absolute; right: -0.04em; bottom: 0; font-family: var(--font-serif);
		font-size: min(48vw, 42rem); font-weight: 900; line-height: 0.8;
		color: rgba(198, 65, 44, 0.035); pointer-events: none; user-select: none;
	}

	.sec {
		padding: clamp(60px, 10vh, 100px) 0; border-top: 2px solid var(--ink);
		position: relative;
	}
	.sec-head {
		display: grid; grid-template-columns: auto 1fr auto; gap: clamp(16px, 2vw, 28px);
		align-items: baseline; margin-bottom: clamp(32px, 5vh, 48px);
	}
	.sec-head .cn {
		font-family: var(--font-serif); font-weight: 900; font-optical-sizing: auto;
		font-size: clamp(3rem, 8vw, 7rem); line-height: 0.82; color: var(--ink);
		letter-spacing: -0.04em;
	}
	.sec-title h2 {
		font-family: var(--font-serif); font-weight: 900; font-optical-sizing: auto;
		font-size: clamp(2rem, 4vw, 3.2rem); line-height: 1; letter-spacing: -0.02em; margin: 0;
	}
	.brush { width: 100%; height: 12px; margin-top: 8px; }
	.folio {
		font-family: var(--font-label); font-weight: 700; font-size: var(--type-label);
		letter-spacing: 0.16em; color: var(--ink-mute);
	}
	...
	.edu-logo { flex: 0 0 auto; border-radius: 4px; object-fit: contain; }
	.edu a { font-weight: 700; color: var(--ink); text-decoration: none; }
	.edu a:hover { color: var(--zhu); }
	.edu .years { font-family: var(--font-label); font-size: 0.7rem; letter-spacing: 0.1em; color: var(--ink-mute); }
	.contrib-embed { margin-top: 32px; grid-column: 1 / -1; }
	...
	.mrow.rev .mviewport { direction: rtl; }
	.mrow.rev .mviewport > div { direction: ltr; }
	.mrow-label {
	...
	.skill {
		font-family: var(--font-label); font-size: clamp(1.4rem, 3vw, 2.2rem);
		font-weight: 900; color: var(--ink-soft); letter-spacing: -0.01em;
	}
	.sep { color: var(--ink-line-strong); font-size: 0.8em; }
	...
	.tl-item {
		...
		padding: 22px 0 30px; border-top: 2px solid var(--ink); position: relative;
	}
	.tl-dot {
		position: absolute; top: 0; left: 120px; width: 10px; height: 10px;
		background: var(--zhu); border-radius: 50%; transform: translate(-50%, -6px);
	}
	.tl-date {
		font-family: var(--font-label); font-size: 0.68rem; font-weight: 700;
	...
	.tl-role { font-family: var(--font-serif); font-weight: 900; font-size: 1.2rem; letter-spacing: -0.01em; margin: 0; }
	.tl-co { font-size: 0.85rem; color: var(--ink-soft); margin: 2px 0 0; }
	.tl-desc { font-size: 0.9rem; color: var(--ink-soft); margin: 8px 0 0; line-height: 1.5; }
	.tl-badges { display: flex; gap: 8px; margin-top: 10px; flex-wrap: wrap; }
	.tl-badges span {
		font-family: var(--font-label); font-size: 0.62rem; letter-spacing: 0.1em;
		text-transform: uppercase; border: 1px solid var(--ink-line); padding: 3px 10px;
		border-radius: 2px; color: var(--ink-mute);
	}

	.exp-open { padding: 32px 0; }
	.exp-status { display: flex; align-items: center; gap: 12px; margin-bottom: 28px; }
	.status-dot {
		width: 8px; height: 8px; border-radius: 50%; background: var(--zhu);
		animation: statusPulse 2s ease-out infinite;
	}
	@keyframes statusPulse {
		60%, 100% { box-shadow: 0 0 0 8px rgba(198, 65, 44, 0); }
	}
	.status-label {
		font-family: var(--font-label); font-size: 0.72rem; font-weight: 700;
		letter-spacing: 0.18em; text-transform: uppercase; color: var(--zhu);
	}
	.exp-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px; margin-bottom: 28px; }
	.exp-stat { display: flex; flex-direction: column; gap: 6px; }
	.exp-num {
		font-family: var(--font-serif); font-weight: 900; font-size: clamp(2rem, 4vw, 3rem);
		line-height: 1; color: var(--ink);
	}
	.exp-cap { font-size: 0.82rem; color: var(--ink-soft); line-height: 1.4; }
	.exp-actions { display: flex; gap: 12px; margin-bottom: 20px; }
	.exp-cta {
		font-family: var(--font-label); font-size: 0.72rem; font-weight: 700;
		letter-spacing: 0.14em; text-transform: uppercase;
		padding: 10px 20px; border: 1px solid var(--ink-line); text-decoration: none;
		color: var(--ink-soft); transition: all 0.25s ease;
	}
	.exp-cta:hover { border-color: var(--zhu); color: var(--zhu); }
	.exp-cta-primary { background: var(--zhu); color: var(--paper); border-color: var(--zhu); }
	.exp-cta-primary:hover { background: var(--ink); border-color: var(--ink); }
	.exp-note { font-size: 0.9rem; line-height: 1.6; color: var(--ink-soft); max-width: 60ch; }

	.work-intro { display: flex; justify-content: space-between; align-items: baseline; margin-bottom: clamp(24px, 4vh, 40px); }
	.work-intro p { font-size: 1.1rem; color: var(--ink-soft); }
	.work-intro span { font-family: var(--font-label); font-size: 0.68rem; letter-spacing: 0.16em; text-transform: uppercase; color: var(--ink-mute); }
	.work { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 0; }
	.row {
		display: grid; grid-template-columns: 56px 1fr; gap: 16px;
		padding: 28px 0; border-top: 1.5px solid var(--ink-line); position: relative;
	}
	.row.featured { grid-template-columns: 56px 1fr 280px; padding: 40px 44px 36px; border-top: 2px solid var(--ink); border-bottom: 2px solid var(--ink); background: linear-gradient(120deg, rgba(198, 65, 44, 0.06), transparent 60%); }
	.row:not(.featured):last-child { border-bottom: 1.5px solid var(--ink-line); }
	.idx {
		font-family: var(--font-label); font-weight: 700; font-size: 0.65rem;
		letter-spacing: 0.14em; color: var(--ink-mute); padding-top: 4px;
		-webkit-text-stroke: 0.5px var(--ink-line);
	}
	.row-title {
		font-family: var(--font-serif); font-weight: 900; font-size: clamp(1.1rem, 2vw, 1.5rem);
		letter-spacing: -0.01em; color: var(--ink); text-decoration: none;
		transition: color 0.3s;
	}
	.row-title:hover { color: var(--zhu); }
	.row-desc { font-size: 0.9rem; color: var(--ink-soft); margin: 6px 0 0; line-height: 1.5; }
	.row-tech {
		font-family: var(--font-label); font-size: 0.65rem; letter-spacing: 0.1em;
		text-transform: uppercase; color: var(--ink-mute); margin: 8px 0 0;
	}
	.row-links { display: flex; gap: 16px; margin-top: 10px; }
	.row-links a {
		font-family: var(--font-label); font-size: 0.68rem; font-weight: 700;
		letter-spacing: 0.1em; text-transform: uppercase; color: var(--ink-soft);
		text-decoration: none; border-bottom: 1px solid transparent; transition: all 0.25s;
	}
	.row-links a:hover { color: var(--zhu); border-bottom-color: var(--zhu); }
	.row-thumb {
		display: block; overflow: hidden; border-radius: 2px;
		border: 1px solid var(--ink-line); text-decoration: none;
	}
	.row-thumb img { width: 100%; height: auto; display: block; transition: transform 0.6s ease; }
	.row-thumb:hover img { transform: scale(1.03); }
	...
	.hx-idx {
		font-family: var(--font-label); font-weight: 700; font-size: 0.65rem;
		letter-spacing: 0.14em; color: var(--ink-mute); padding-top: 6px;
	}
	.hx-title { font-family: var(--font-serif); font-weight: 900; font-size: 1.3rem; letter-spacing: -0.01em; margin: 0; }
	.hx-meta {
		font-family: var(--font-label); font-size: 0.65rem; letter-spacing: 0.1em;
		text-transform: uppercase; color: var(--ink-mute); margin: 4px 0 0;
	}
	.hx-desc { font-size: 0.9rem; color: var(--ink-soft); margin: 8px 0 0; line-height: 1.5; }
	.hx-links { display: flex; gap: 16px; margin-top: 12px; flex-wrap: wrap; }
	.hx-links a {
		font-family: var(--font-label); font-size: 0.68rem; font-weight: 700;
		letter-spacing: 0.1em; text-transform: uppercase; color: var(--ink-soft);
		text-decoration: none; border-bottom: 1px solid transparent; transition: all 0.25s;
	}
	.hx-links a:hover { color: var(--zhu); border-bottom-color: var(--zhu); }
	.repos {
		margin-top: 28px; font-family: var(--font-label); font-size: 0.72rem;
		letter-spacing: 0.14em; text-transform: uppercase; color: var(--ink-soft);
	}
	.repos .count { color: var(--ink); font-weight: 900; }

	.anime-head { display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; }
	.anime-head .mini-h { margin: 0; }
	.c-nav { display: flex; gap: 8px; }
	.c-arrow {
		width: 36px; height: 36px; display: grid; place-items: center;
		border: 1px solid var(--ink-line); background: transparent; color: var(--ink-soft);
		font-size: 1rem; cursor: pointer; border-radius: 2px; transition: all 0.25s;
	}
	.c-arrow:hover { border-color: var(--zhu); color: var(--zhu); }
	.track {
	...
	.acard { flex: 0 0 auto; scroll-snap-align: start; width: 260px; }
	.acard a { text-decoration: none; color: inherit; display: block; }
	.frame {
		position: relative; overflow: hidden; border-radius: 2px;
		border: 1px solid var(--ink-line); aspect-ratio: 280 / 373;
	}
	...
	.anime-no {
		...
		background: rgba(26, 24, 21, 0.6); padding: 3px 8px; border-radius: 2px;
	}
	.anime-quote {
		position: absolute; bottom: 0; left: 0; right: 0;
		padding: 12px; font-size: 0.78rem; font-style: italic; color: var(--paper);
	...
	.track-foot { display: flex; align-items: center; gap: 16px; margin-top: 12px; }
	.track-bar { flex: 1; height: 2px; background: var(--ink-line); overflow: hidden; }
	.track-bar-fill { height: 100%; background: var(--zhu); transform-origin: 0 50%; }
	.drag-hint { font-family: var(--font-label); font-size: 0.62rem; letter-spacing: 0.1em; text-transform: uppercase; color: var(--ink-mute); margin: 0; }
	.off-manifesto {
		margin-top: 48px; padding: clamp(32px, 6vh, 48px) 0;
		border-top: 2px solid var(--ink); border-bottom: 2px solid var(--ink);
		display: flex; flex-direction: column; gap: 6px;
	}
	.off-manifesto span { font-family: var(--font-label); font-size: var(--type-label); letter-spacing: 0.24em; text-transform: uppercase; color: var(--ink-mute); }
	.off-manifesto strong { font-family: var(--font-serif); font-weight: 900; font-size: clamp(1.8rem, 4vw, 3rem); letter-spacing: -0.02em; color: var(--ink); }
	.off-manifesto em { font-style: italic; font-size: 0.9rem; color: var(--ink-soft); }

	.favorites-head {
		display: grid; grid-template-columns: 1fr 1fr; gap: 16px;
		align-items: end; margin-top: clamp(40px, 7vh, 64px); margin-bottom: 24px;
	}
	.favorites-head h3 {
		font-family: var(--font-serif); font-weight: 900; font-size: clamp(1.3rem, 2.5vw, 2rem);
		line-height: 1.2; letter-spacing: -0.01em; margin: 8px 0 0;
	}
	.favorites-head p { font-size: 0.9rem; color: var(--ink-soft); line-height: 1.5; }
	.favs-mosaic {
		display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; list-style: none;
		margin: 0; padding: 0;
	}
	.fav { position: relative; }
	.fav-link { text-decoration: none; color: inherit; display: block; position: relative; overflow: hidden; border-radius: 2px; }
	.fav-img { overflow: hidden; aspect-ratio: 16 / 9; }
	.fav-img img { width: 100%; height: 100%; object-fit: cover; display: block; transition: transform 0.6s ease; }
	.fav:hover .fav-img img { transform: scale(1.04); }
	.fav-cap {
		position: absolute; inset: 0; padding: 16px; display: flex; flex-direction: column;
		justify-content: flex-end; background: linear-gradient(transparent 40%, rgba(26, 24, 21, 0.75));
		opacity: 0; transition: opacity 0.35s ease;
	}
	.fav:hover .fav-cap { opacity: 1; }
	.fav-index {
		font-family: var(--font-label); font-weight: 700; font-size: 0.6rem;
		letter-spacing: 0.12em; color: var(--paper); opacity: 0.7;
	}
	.fav-cap b {
		font-family: var(--font-serif); font-weight: 900; font-size: 1rem;
		color: var(--paper); margin-top: 4px;
	}
	.fav-cap span { font-size: 0.75rem; color: var(--paper); opacity: 0.85; margin-top: 2px; }
	.fav-cap em { font-size: 0.65rem; font-style: normal; color: var(--zhu); margin-top: 4px; }
	.fav-shimmer {
	...
	.fav.featured-fav { grid-column: 1 / -1; }
	...
	.contact .cn.ghost {
		font-family: var(--font-serif); font-weight: 900; font-optical-sizing: auto;
		font-size: clamp(6rem, 18vw, 16rem); line-height: 0.7; color: var(--ink-line);
		letter-spacing: -0.04em;
	}
	.c-tag {
		font-family: var(--font-label); font-size: var(--type-label); letter-spacing: 0.32em;
		text-transform: uppercase; color: var(--zhu); margin: 0 0 16px;
	}
	.say {
		font-family: var(--font-serif); font-weight: 900; font-optical-sizing: auto;
		font-size: clamp(2rem, 5vw, 3.5rem); line-height: 1; letter-spacing: -0.02em;
		margin: 0 0 24px;
	}
	.email-wrap { margin: 0 0 32px; }
	.email {
		font-family: var(--font-serif); font-weight: 700; font-size: clamp(1.2rem, 2.5vw, 1.8rem);
		color: var(--ink); text-decoration: none; border-bottom: 2px solid var(--zhu);
		padding-bottom: 2px; transition: color 0.25s;
	}
	.email:hover { color: var(--zhu); }
	.socials { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 0; }
	.social-link {
		display: flex; align-items: center; gap: 12px; padding: 12px 0;
		border-top: 1px solid var(--ink-line); text-decoration: none; color: var(--ink-soft);
		transition: color 0.25s; cursor: pointer;
	}
	.social-link:last-child { border-bottom: 1px solid var(--ink-line); }
	.social-link:hover { color: var(--zhu); }
	.social-icon { width: 22px; height: 22px; flex: 0 0 auto; }
	.social-name { font-family: var(--font-label); font-size: 0.72rem; font-weight: 700; letter-spacing: 0.14em; text-transform: uppercase; }
	.social-arrow { margin-left: auto; font-size: 1.1rem; }

	.colophon {
		display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 24px;
		padding: 24px 0; margin-top: clamp(48px, 9vh, 80px);
		border-top: 2px solid var(--ink);
		font-family: var(--font-label); font-size: 0.68rem;
		letter-spacing: 0.14em; text-transform: uppercase; color: var(--ink-soft);
	}
	.col-mark { color: var(--zhu); margin-right: 8px; }
	.col-title { font-weight: 700; color: var(--ink); }
	...
	@media (max-width: 980px) {
		...
		.favs-mosaic { grid-template-columns: repeat(2, 1fr); }
		.colophon { grid-template-columns: 1fr; text-align: left; }
		.contact { grid-template-columns: 1fr; }
		.contact .cn.ghost { font-size: clamp(4rem, 14vw, 8rem); }
	}

	@media (max-width: 640px) {
		.work-intro { flex-direction: column; gap: 8px; }
		.exp-grid { grid-template-columns: 1fr; }
		.hx { grid-template-columns: 1fr; }
		.tl-dot { display: none; }
		.tl-item { grid-template-columns: 1fr; padding-left: 16px; }
	}
</style>
