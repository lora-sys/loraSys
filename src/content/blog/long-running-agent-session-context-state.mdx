...
<ArchitectureStack
  title="长任务的三层状态"
  layers="环境状态::系统真实留下的事实::代码文件、Git、数据库、测试结果和 artifact 应该能独立于会话存在|任务状态::跨会话继续工作的进度::目标、完成项、未完成项、决定和下一步需要持久保存|工作上下文::这一轮推理需要的信息::每一轮只取当前任务真正需要的高信号上下文，不把全部历史重新塞回模型"
  source="结合 OpenAI Agents API 与 Anthropic Managed Agents 的状态边界整理"
/>
...
下面是通用工程建议，不对应某个已核验的仓库实现。示例模块和字段只用于说明长会话状态应该怎样拆分，不能当作现有项目已经实现的功能。
