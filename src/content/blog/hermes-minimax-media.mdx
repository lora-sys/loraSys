...
description: '记录 Hermes 的 MiniMax 图片与视频插件实现，包括 image-01、Hailuo 异步任务、CN 与 global endpoint fallback、端到端测试和 PyPI 发布。'
...
这篇记录如何把 MiniMax 的图片和视频生成能力接入 Hermes，并在一个 pip 包中提供图片与视频两个后端。实现覆盖文生图、图生图、文生视频、图生视频和人物参考视频。

文中的返回时间、文件大小和分辨率来自当时的端到端测试记录。API 行为和模型限制会变化，实际接入时仍应以当前官方文档和接口返回为准。

## 为什么选 MiniMax

Hermes Agent 内置的 `image_generate` 工具原本只支持少数几个海外后端（FAL、xAI、OpenAI、Krea、Replicate）。这次接入 MiniMax，主要为了补齐两个能力：

- **`image-01`**：单 API 同时支持文生图和图生图（`subject_reference` 单参考图），可以复用同一套 Hermes 图片后端接口
- **Hailuo 系列视频模型**：采用异步任务模式，覆盖文生视频、图生视频、首尾帧和人物参考等路径。本文实际跑通的是 Hailuo-2.3 的 6 秒 768P 文生视频

而且 MiniMax 有两个 endpoint：

| 区域               | 域名               | 用户中心                   |
| ------------------ | ------------------ | -------------------------- |
| **国内（CN）**     | `api.minimaxi.com` | [api.minimaxi.com](https://api.minimaxi.com) |
| **国际（global）** | `api.minimax.io`   | [minimax.io](https://www.minimax.io)   |

插件会自动选：CN 优先（你用的是国内账户就走国内），global 作为兜底。

## 实现一个插件要做什么

Hermes 的 `ImageGenProvider` 和 `VideoGenProvider` 已经封装环境变量加载、URL 缓存和错误格式化。这个插件主要补两个 provider 的请求与响应逻辑：
...
一个 API 参数兼容问题：官方文档示例里写的是 `720P`，但**实际 API 不接受**。提交 `720P` 会立刻报错 `does not support resolution 720P, supported resolutions: 768P, 1080P`。我的插件把 `720P` 自动 coerce 成 `768P`，调用方可以无感。
...
<StatStrip
  stats="5s::image-01 T2I 返回|86s::Hailuo-2.3 T2V 完成|20::pytest 通过"
  source="本文端到端验证记录。时间只代表这次本地实测，不代表服务 SLA。"
/>
...
<ComparePanel
  title="两个 Hermes 媒体后端的侧重点"
  leftTitle="StepFun"
  left="step-image-edit-2 在本文记录里约 1–2 秒，适合快速迭代|图编辑是这篇 StepFun 插件的重点能力|需要按账户类型选择计费 endpoint"
  rightTitle="MiniMax"
  right="image-01 同时覆盖文生图与单参考图图生图|同一插件族增加 Hailuo 视频能力|本文验证了 CN endpoint 自动选择"
  source="依据两篇 Hermes 插件文章里的实际实现与验证记录"
/>

而且两个插件可以**同时启用**，在 `config.yaml` 里改 `image_gen.provider` 就能切换，不需要卸载任何一个。

<Callout tone="warning" title="StepFun 的计费路径另算">
StepFun 的 `/v1/` 和 `/step_plan/v1/` 使用同一个 key 也可能都认证成功，但分别走账户余额和订阅 Credit。切换 provider 时要同时检查 endpoint 与账单来源。完整排查记录在 [StepFun 文章](/blog/hermes-stepfun-imagegen) 和对应 README。
</Callout>
...
项目包已发布为 [`hermes-minimax-media`](https://github.com/lora-sys/hermes-minimax-media)。仓库保留安装方式、当前支持范围和后续适配项。
