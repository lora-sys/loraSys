...
<EvidenceFlow
  title="一次受控 Agent 运行要留下什么"
  items="意图::先明确用户真正允许执行的动作和目标|权限::在工具调用前检查当前 Agent 可以访问什么|执行::让模型和工具完成任务，但限制时间、次数和预算|证据::保留工具输入、结果、审批与关键状态变化|评测::用可重复的检查判断结果是否达到要求"
  source="根据本文讨论的 Harness 控制面整理"
 />
