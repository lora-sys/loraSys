...
<ComparePanel
  title="同一次运行里的两种消息职责"
  leftTitle="流式增量"
  left="尽快更新页面上的文字和状态|允许用户看到运行进度|不能单独作为会话恢复依据"
  rightTitle="最终消息"
  right="保存完整助手输出和工具调用信息|用于恢复会话与构造下一次模型请求|不需要替代前端的实时增量显示"
  source="依据 Tau Agent Loop 文档中 MessageUpdateEvent 与 MessageEndEvent 的职责划分"
/>
