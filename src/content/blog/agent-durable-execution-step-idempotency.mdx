---
title: 'Agent 跑到一半进程挂了，为什么不能直接重跑？读懂持久执行、Step 边界与幂等性'
description: '解释 Agent 崩溃恢复中的持久执行、Step 边界、Run 与 Step 幂等，以及外部副作用为什么仍需要稳定幂等键。'
publishDate: '2026-09-19T00:00:00Z'
tags: ['AI', 'Agent', 'Durable Execution', 'Workflow', 'Idempotency']
language: 'zh-CN'
draft: false
---
...
主材料是 Vercel AI SDK 的 [WorkflowAgent 官方文档](https://github.com/vercel/ai/blob/main/content/docs/03-agents/07-workflow-agent.mdx)。它把 Agent 的模型调用、工具执行、暂停和恢复放进持久工作流。相关材料是 Vercel Workflow 的 [Workflows and Steps](https://github.com/vercel/workflow/blob/main/docs/content/docs/v5/foundations/workflows-and-steps.mdx)、[Idempotency](https://github.com/vercel/workflow/blob/main/docs/content/docs/v5/foundations/idempotency.mdx) 和 [AI SDK integration](https://github.com/vercel/workflow/blob/main/docs/content/docs/v5/cookbook/integrations/ai-sdk.mdx)。AI SDK 的 WorkflowAgent 代码在 2026 年 9 月 18 日仍有提交和包更新，所以今天读的是当前实现正在使用的问题，不是把旧材料包装成新闻。
...
![进程崩溃后的普通内存循环与持久执行对比](../../assets/blog/agent-durable-execution-step-idempotency/2026-09-19-durable-restart.svg)
...
![整轮作为一个 Step 与副作用工具单独成 Step 的重试范围对比](../../assets/blog/agent-durable-execution-step-idempotency/2026-09-19-step-boundary.svg)
...
![没有幂等键与使用稳定幂等键的外部副作用对比](../../assets/blog/agent-durable-execution-step-idempotency/2026-09-19-idempotency.svg)
...
现有 `extensions/glassbox/trace-hooks.ts` 已经记录 `agent_start`、`agent_end`、`turn_start`、`turn_end`、`tool_call`、`tool_result` 和 usage。这是很好的执行信号。不过当前实现把事件写进进程内的 `traceBuffer`。进程结束以后，这个数组不会成为持久恢复依据。[trace-hooks.ts](https://github.com/lora-sys/lora-pi-kit/blob/main/extensions/glassbox/trace-hooks.ts)
...
![Lora PI Kit 现有边界接入持久 Run 的工程迁移示意](../../assets/blog/agent-durable-execution-step-idempotency/2026-09-19-pikit-durable-layer.svg)
