...
updatedDate: '2026-09-17T06:44:08Z'
...
主材料是 Kitaru 的 Replay、Evaluator 和 Regression Suite。仓库由 `zenml-io` 维护，默认开发分支为 `develop`，许可证为 Apache 2.0。2026 年 9 月 16 日仓库仍有功能和依赖更新。本文还用 Anthropic 的 Agent Eval 方法，以及 OpenAI 对评测数据质量和生产反馈闭环的公开文章交叉检查。本文没有复现 Kitaru 的完整实验环境，也不把项目方的产品描述当成独立验证。[Kitaru 官方仓库](https://github.com/zenml-io/kitaru)
...
![把线上失败变成回归测试的链路](../../assets/blog/kitaru-agent-regression-testing/2026-09-16-regression-loop.svg)

图 1 先看最左和最右。线上失败留下 Session，固定一批 Session 成为 Cohort。基线先重放一次。确认现场能稳定复现以后，只改一个变量，再让候选版本跑同一批任务。评测没有通过，CI 就不放行。
...
![Kitaru 重放中的工具策略](../../assets/blog/kitaru-agent-regression-testing/2026-09-16-tool-policy.svg)
...
![Outcome 与 Transcript 的评分顺序](../../assets/blog/kitaru-agent-regression-testing/2026-09-16-outcome-transcript.svg)
...
![回归测试集的三个来源](../../assets/blog/kitaru-agent-regression-testing/2026-09-16-regression-cohort.svg)
...
Kitaru 官方示例建议 PR 用较小 cohort，定时任务用更大的流量样本。发现新失败后，把这个 session 加到新的 cohort version，之后的测试都会包含它。[Regression Suite 官方文档](https://github.com/zenml-io/kitaru/blob/develop/docs/book/guides/regression-suite.md)

## 迁移到 Zhihu Threads eval v3

Zhihu Threads eval v3 已经把不少基础工作做好。当前记录显示，execution input 和 expected 已经分开。每次尝试会保留。引文存在、来源绑定和语义支持分别检查。逐轮证据快照也会保存。
...
[Kitaru 官方仓库](https://github.com/zenml-io/kitaru)：核对项目维护方、当前代码和许可证。

[Kitaru Replay](https://github.com/zenml-io/kitaru/blob/develop/docs/book/concepts/replay.md)：核对 Replay、fork、工具策略和当前限制。

[Kitaru Evaluators](https://github.com/zenml-io/kitaru/blob/develop/docs/book/concepts/evaluators.md)：核对 Evaluation 数据结构、版本和批量评测方式。

[Kitaru Regression Suite](https://github.com/zenml-io/kitaru/blob/develop/docs/book/guides/regression-suite.md)：核对固定 cohort、experiment、CI gate 和长期维护方式。
...
本文没有运行 Kitaru，也没有复现它的端到端示例。Kitaru 的功能说明来自官方仓库和文档。工程迁移部分基于这些机制和当前 Zhihu Threads eval v3 状态。
