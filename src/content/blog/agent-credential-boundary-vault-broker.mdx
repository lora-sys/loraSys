...
<ComparePanel
  title="同一个 API 请求，两种凭据路径"
  leftTitle="Secret 进入 sandbox"
  left="真实 token 出现在执行环境|生成代码可以直接读取|调试或异常日志可能带出原始值"
  rightTitle="Host boundary 注入"
  right="sandbox 只持有占位值或无原始值|Host 先校验目标与策略|真实 token 只加入批准的出站请求"
  source="依据本文引用的 OpenAI Sandbox Security 与 Vercel Sandbox Header Injection 文档"
/>
