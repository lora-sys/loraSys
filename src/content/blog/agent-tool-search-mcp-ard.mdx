...
<ProcessSteps
  title="一轮 Tool Search 的实际路径"
  steps="理解任务::先判断当前问题需要哪类能力|搜索工具目录::用名称、描述和参数信息缩小候选范围|加载完整定义::只把命中的工具 Schema 放回当前上下文|调用并校验::执行工具后继续检查权限、结果和错误"
  source="根据 Anthropic Tool Search 文档和本文机制拆解"
/>
