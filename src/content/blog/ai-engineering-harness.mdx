---
title: 'AI Engineering Harness：把 AI 代码放进可验证的工程闭环'
description: '介绍一个多 Agent 工程工作流，如何用独立 worktree、CI、冷启动审查、证据门和分层上下文控制 AI 生成代码的交付。'
...
AI 可以很快生成一批能运行的代码，但工程交付还需要测试、CI、审查、文档和可追溯的验证结果。问题不在代码生成速度，而在生成结果怎样进入一个可以继续维护的流程。

例如，用户认证功能即使能启动，也仍要检查 Cookie 策略、权限边界、测试覆盖和回归路径。把这些工作留到最后人工补，会让每次改动都重新经历同一轮检查。

AI Engineering Harness 的目标是把这些检查变成固定流程。Agent 可以实现功能，但合并条件由测试、审查和证据决定。
...
测试、CI、文档、安全审查和代码审查往往分散在不同步骤。具体耗时和成本取决于仓库规模、测试条件与模型价格，本文不把原来的示例天数和 token 成本当作测量结果。

## Harness 如何组织工程闭环

<Callout tone="note" title="目标">
代码是否进入 main 由 CI、审查和证据决定，不由 Agent 自己宣布完成。
</Callout>

### 核心工作方式

AI Engineering Harness 把仓库接管、Issue 分工、Worktree、CI、审查和证据目录组织成一套固定流程。当前仓库定义了 18 类 Agent 角色，但这些角色是实现细节，不是必须照搬的行业标准。
...
当前流程把 `CI Pass`、至少 2 名冷启动审查员批准和证据完整作为进入 `main` 的条件。

<StatStrip stats="18::Agent 角色|2+::冷启动审查员|4::上下文层级" source="本文架构定义" />
...
graph TD
    ...
    Coordinator -->|分派| Backend[Backend Agent]
    Coordinator -->|分派| Database[Database Agent]
    ...
    Merge -->|阶段总结| Memory[Project memory]
    Memory -->|沉淀| Next[Next issue]
```
...
<Callout tone="data" title="记录口径">
下表同时包含仓库记录和明确标为“示意”的案例。示意数字不当作项目实测。
</Callout>
...
这套 Harness 适合多 Issue、需要多角色审查、需要保留证据或经常由不同 Agent 接手的仓库。它不适合把一次性小改动包装成复杂流程。角色数量、审查人数和上下文层级都应该按仓库风险调整，而不是照搬默认配置。

开始接管一个仓库时，先做 Quick Scan，再选一个真实 Issue 跑完整闭环。验收时检查 CI、审查记录和 `docs/evidence/<issue>/` 是否对应同一份改动。只有这条链能稳定重复，才值得增加并行 Agent 或更多自动化。
